...
DEVICE_INFO = {
    "model": "matter.light.color_temperature",
    "display_name": "Matter Color Temperature Light",
    "manufacturer": "Matter",
    "category": "light",
    "platform": "matter",
}
...
ACTION_SPECS = [
    ...
    {"action": "set_color_temperature", "description": "Set color temperature in kelvin.", "args": "kelvin", "range": "2000~6500"},
]
...
STATUS_FIELDS = [
    ...
    {"field": "color_temperature_kelvin", "description": "Current color temperature in kelvin.", "type": "int or null"},
...
def _mired_to_kelvin(mired: Any) -> Any:
    if mired in (None, 0):
        return None
    try:
        return round(1000000 / float(mired))
    except (TypeError, ValueError, ZeroDivisionError):
        return None


class MatterColorTemperatureLight:
    ...
    def _color_temp_bounds(self) -> tuple[int, int]:
        min_mired = self.client.read_device_attribute(self.did, self.client.COLOR_CONTROL_CLUSTER_ID, 0x400B)
        max_mired = self.client.read_device_attribute(self.did, self.client.COLOR_CONTROL_CLUSTER_ID, 0x400C)
        lower = 153 if min_mired is None else int(min_mired)
        upper = 500 if max_mired is None else int(max_mired)
        return lower, upper
    ...
    def query_status(self) -> str:
        ...
        kelvin = _mired_to_kelvin(self.client.read_device_attribute(self.did, self.client.COLOR_CONTROL_CLUSTER_ID, 7))
        return (
            f"power: {power}, "
            f"brightness_pct: {brightness_pct}, "
            f"color_temperature_kelvin: {kelvin}, "
    ...
    def set_color_temperature(self, kelvin: Any) -> str:
        try:
            target_kelvin = int(kelvin)
        except (TypeError, ValueError):
            return "Error: kelvin must be an integer between 2000 and 6500."
        if not 2000 <= target_kelvin <= 6500:
            return "Error: kelvin must be between 2000 and 6500."

        target_mired = _kelvin_to_mired(target_kelvin)
        min_mired, max_mired = self._color_temp_bounds()
        target_mired = max(min_mired, min(max_mired, target_mired))

        self.client.invoke_device_command(
            self.did,
            self.client.COLOR_CONTROL_CLUSTER_ID,
            "MoveToColorTemperature",
            payload={
                "colorTemperatureMireds": target_mired,
                "transitionTime": 0,
                "optionsMask": 0,
                "optionsOverride": 0,
            },
        )
        return f"color temperature set to {target_kelvin}K."
    ...
    def perform_action(self, action: str, **kwargs) -> str:
        ...
        if action == "set_color_temperature":
            return self.set_color_temperature(kwargs.get("kelvin"))
        if action == "query_status":
            return self.query_status()
        return f"Error: unsupported action '{action}'."


DeviceClass = MatterColorTemperatureLight
